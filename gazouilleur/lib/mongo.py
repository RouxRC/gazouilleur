...
def ensure_indexes():
    ...
    yield db.ensure_index('feeds', sortasc('channel') + sortasc('database'), background=True)
    yield db.ensure_index('feeds', sortasc('channel') + sortasc('database') + sortdesc('timestamp'), background=True)
    yield db.ensure_index('filters', sortasc('channel'), background=True)
    yield db.ensure_index('filters', sortasc('channel') + sortasc('keyword') + sortdesc('timestamp'), background=True)
    yield db.ensure_index('news', sortasc('channel') + sortdesc('_id'), background=True)
    ...
    yield db.ensure_index('dms', sortasc('id') + sortasc('channel'), background=True)
    yield db.ensure_index('tweets', sortasc('id'), background=True)
